...
export default function PopularTrips({ scrollProgress, onPlaceholderPositionsChange, popularTripsRef, isMobileOrTablet }: PopularTripsProps) {
  ...
  const CardComponent = ({ trip, gridIndex, isGrid = false }: { trip: Trip; gridIndex: number; isGrid?: boolean }) => {
    const motionProps = isMobileOrTablet 
      ...
      : {
        ...
        // FIX: Match the size changes in HeroSection for smooth transition
    ...
    return (
      <motion.div
        ...
        className={`bg-white/95 backdrop-blur-md rounded-lg overflow-hidden shadow-lg hover:shadow-purple-500/30 transition-all ${widthClasses} h-52 xl:h-56 cursor-pointer flex flex-col`}
  ...
  return (
    <section id="popular-trips" className="py-12 sm:py-16 lg:py-24 relative overflow-hidden lg:overflow-visible bg-gradient-to-b from-background via-muted/20 to-background" ref={popularTripsRef}>
      ...
      {/* FIX: WRAPPED BLOBS IN OVERFLOW HIDDEN CONTAINER */}
      ...
      {isMobileOrTablet ? (
        <div className="relative px-4 sm:px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-4 md:gap-5 max-w-7xl mx-auto">
            {popularTrips.slice(0, 6).map((trip, idx) => (
              <CardComponent key={trip.id} trip={trip} gridIndex={idx} isGrid={true} />
            ))}
          </div>
        </div>
      ) : (
      ...
      <Dialog open={showAllTripsModal} onOpenChange={setShowAllTripsModal}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto mx-auto">
